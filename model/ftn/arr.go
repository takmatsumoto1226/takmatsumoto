...
func (fa FTNArray) FeatureBinaryCSVExport(fn string) {
	// 建立 CSV 檔案
	file, err := os.Create(fn)
	if err != nil {
		panic(err)
	...
	// 將資料寫入 CSV
	for _, f := range fa {
		if err := writer.Write(f.Feature.SplitBinaryArray(1)); err != nil {
			panic(err)
		}
	}
...
func (fa FTNArray) FeatureCSVExport(fn string) {
	// 建立 CSV 檔案
	file, err := os.Create(fn)
	if err != nil {
		panic(err)
	...
	// 將資料寫入 CSV
	for _, f := range fa {
		if err := writer.Write(f.Feature.SplitKeyArray(1)); err != nil {
			panic(err)
		}
	}
